...
pub async fn run(cmd: &mut DeleteArgs) -> Result<bool> {
    ...
    let mut tool_section = config.tool;

    // Check for the `[tool.utpm]` subsection.
    if let Some(entry) = tool_section.sections.get_mut("utpm") {
        ...
        if let Some(mut dependencies) = extra.dependencies.take() {
            ...
            tool_section
                .sections
                .insert("utpm".into(), Map::try_from(extra)?);
        } else {
            utpm_log!(
        ...
        utpm_log!(
            "Nothing has changed! There isn't a tool section dedicated to utpm in the typst.toml."
        )
    }
    config.tool = tool_section;
