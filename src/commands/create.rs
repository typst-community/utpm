...
pub fn run(cmd: &mut CreateArgs) -> Result<bool> {
    ...
    if let Some(auts) = &cmd.authors {
        for e in auts {
            authors.insert(Author::from_str(&e).unwrap());
        }
    }
    ...
    let mut pkg = Package {
        name: Ident::from_str(cmd.name.to_owned().unwrap_or("temp".into()).as_str()).unwrap(),
        version: cmd.version.to_owned(),
        entrypoint: cmd.entrypoint.to_owned().into(),
        authors,
        license: License::from_str(cmd.license.to_owned().unwrap().as_str()).unwrap(),
        description: cmd.description.to_owned().unwrap_or("".into()),
        repository: Some(
            Website::from_str(cmd.repository.to_owned().unwrap_or("".into()).as_str()).unwrap(),
        ),
        homepage: Some(
            Website::from_str(cmd.homepage.to_owned().unwrap_or("".into()).as_str()).unwrap(),
        ),
        keywords,
    ...
    keys.insert("utpm".into(), Table::try_from(extra).unwrap());
    Manifest {
        package: pkg,
        tool: Some(Tool { keys }),
        template: None,
